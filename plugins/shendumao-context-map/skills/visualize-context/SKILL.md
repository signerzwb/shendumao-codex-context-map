...
description: 把当前上下文、宿主授权读取的指定 Codex 任务或用户提供的规划整理并持久保存为神都猫脉络图，支持增量编辑与版本回看。当用户想回顾初心、梳理主题分支、查看决定原因、追踪未完成事项或继续已有地图时使用。
---

# 神都猫脉络

把线性聊天整理成可追溯、可继续生长的结构，但不要改写或伪造证据。

## 工作流

1. 确认来源与范围。当前内容已在上下文中时可以直接整理；对于其他 Codex 任务，只有用户明确指定且宿主实际提供任务读取能力时才能读取。插件本身不会自动获得任务历史；无法读取时说明边界，请用户提供内容，不要声称已经看过。
...
7. 文字结果中同时给出最重要的目标、转折和未完成事项，使不支持图形界面的宿主也能完成任务。
...
用户只想体验界面时，直接调用 `render_context_map`，省略 `mapId` 即可打开只读演示图。查找本机已保存地图时使用 `list_context_maps`。
